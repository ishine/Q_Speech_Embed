...
model.load_state_dict(torch.load("models/best_model_0.8128.pth", map_location="cpu"))
...
def extract_named_modules_with_params(model):
    return [(name, mod) for name, mod in model.named_modules()
            if any(k in mod.state_dict() for k in ("weight", "bias"))]
...
for name, module in extract_named_modules_with_params(model):
    ...
    extracted_data.append((name, weight_shape, bias_shape))
